...
- **金币入袋**：每次冒出收入数字，同时弹出一枚金币飞进小金袋

#### 💰 氛围特效（小金袋 / 金笔 / 星星）
- **小金袋**：收入框右上角的钱袋随今日进度慢慢变鼓，过 1/3、2/3 袋口会冒出金币，装满后发光
- **开场动画**：打开弹窗时，离开期间赚的钱化作金币飞进钱袋，数字从上次看到的金额滚到当前金额，并提示"离开 40 分钟，又进账 ¥56.30"（当天首次打开提示"今天已经进账 ¥xx"）
- **金笔打勾**：每多挣够一个价值物品（辣翅、可乐等），金笔在"约等于"旁边打个勾，"+1 🍗"飞进钱袋
- **小星星**：收入数字周围零星闪烁，越接近下班越密，不会挡住数字
- **进度里程碑**：25% / 50% / 75% 在进度条对应位置迸出星星和金币；100% 时钱袋炸开庆祝。每天每档只放一次，下班那一刻没开弹窗的话，会在下班后第一次打开时补放
- **不打扰**：佛系模式下全部关闭；系统开启"减少动态效果"时只保留钱袋和文字提示
...
│   │   ├── effects.js      # 氛围特效（小金袋、金币、金笔、星星、里程碑）
│   │   ├── effects.css     # 氛围特效样式
...
│   └── assets/             # 静态资源
├── public/                 # 公共资源
├── manifest.json           # Chrome插件配置文件 (v3.1.0)
├── package.json            # 项目依赖配置 (v3.1.0)
