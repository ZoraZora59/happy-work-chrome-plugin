...
- **月薪优先**：用户只填月薪、年终奖即可，App 自动折算有效时薪；每月计薪天数不用手填——
  按「每周工作制」直接数出当月的实际工作天数（双休 / 单休 / 大小周 / 自定义都自动覆盖），
  收入规则里只读展示「本月 N 天」。旧版时薪配置会自动迁移成月薪估算值（年终奖按全年 /12 摊到每月）。
- **作息可配置**：默认 965（午休 12:00-13:30、晚休 18:30-19:30），一键切 996 或自定义上下班时间。
  上下班、午休、晚休都用原生紧凑时间选择（点一下弹系统滚轮，5 分钟一档），整页只占几行、手指好对齐。
...
- 配置（月薪、年终奖、每周工作制、作息、午休/晚休、加班倍数）与进行中的会话用
  **UserDefaults** 持久化；每月计薪天数按工作制实时推算，不落库。
...
`HappyWorkTests` 是纯逻辑单测（无需界面），共 20 个用例，覆盖：月薪按当月实际工作天数折算、
年终奖按全年折算、按工作制数出当月计薪天数、作息分钟对齐 5 分钟刻度、作息摘要、日夜主题持久化、
中文倒计时格式化、午休扣除、晚休扣除、下班后加班费、每周工作制（双休/单休/大小周/自定义）、
